    ...
    var _ui = window._ui = {
        ...
        _successSubProductosAlmacenamiento: function (data) {
            ...
            $.each(data, function (i, values) {
                _content.append(_ui.createElement("li", { "value": values.Value,
                    "id": "li_" + i,
                    "data-selected": "falso",
        ...
        selectSubproducto: function (e) {
            ...
            if (seleted == "falso") {
                ...
                _this.find("input").css({ "background-color": "#9AAE04",
                    "color": "White"
                });
                _this.css({ "background-color": "#9AAE04",
                    "color": "White"
                });
        ...
        configCampo: function (content, data) {
            ...
            $.each(data, function (i, values) {
                ...
                $("#tdText" + i).append(_ui.createElement("label", "", values.Descripcion + ":"));
                $("#tdValue" + i).append(_ui.createElement(_ui.validarTipoElemento(values.TcId),
                                                 { "type": _ui.validTypeInput(values.TcId),
                                                     //  "name": "_value",
                                                     "name": "idCampo_" + values.CampoId,
                                                     "style": "width:" + values.CampAncho + "%",
                                                     "id": $.trim(((values.Descripcion).replace(/\s/g, '')).replace(/\./g, ''))
                                                 }, ""));


                switch (values.TcId) {
                    case 5:
                        var idCampo = $("#" + $.trim(((values.Descripcion).replace(/\s/g, '')).replace(/\./g, '')));
                        if (_ui.validarTipoElemento(values.TcId).match(/select/g) == "select") {
                            idCampo.combobox();
                            $.ajax({
                                "type": "GET",
                                "url": values.Descripcion == "Estado" ? "/Contabilizar/_GetDropDownListEstados" : "/Radicacion/_GetValoresList?CampoId=" + values.CampoId,
                                "success": function (data) {
                                    $(document).ready(function () {
                                        ...
                                        if (values.CampObligatorio == true) {
                                            $(padre).find("input").attr({ "required": "",
                                                "title": ""
                                            });
                                        }
                            ...
                            if ((values.Descripcion).match(/Sociedad/g) == "Sociedad") {
                                idCampo.attr("data-work", "sociedad");
                            }

                            if ((values.Descripcion).match(/Causal/g) == "Causal") {
                                idCampo.attr("data-work", "causal");
                            }
                        }
                        break;
                    case 3:
                        _ui.idCamp = $("#" + $.trim(((values.Descripcion).replace(/\s/g, '')).replace(/\./g, '')));
        ...
        configCampoData: function (Campos, Data) {
            ...
            $.each(Campos, function (i, values) {
                $.each(Data, function (j, values2) {
                    if (values2.CampoId == values.CampoId) {
                        var idCampo = $("#" + $.trim(((values.Descripcion).replace(/\s/g, '')).replace(/\./g, '')));
        ...
        builModelDialog: function (elemento) {
